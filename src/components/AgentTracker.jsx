...
import { useTracker, AVOIDABLE_CATEGORIES, ACTIVITY_TYPES } from '../lib/useTracker';
import { Plus, CheckCircle, Copy, Check, Sun, Moon, Search, ChevronLeft, X, Calendar } from 'lucide-react';
...
function CaseDetail({ c, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="rounded-2xl border w-full max-w-md" style={{ background:'var(--bg-card)', borderColor:'var(--border)' }}>
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor:'var(--border)' }}>
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold" style={{ color:'var(--text-primary)' }}>{c.case_number.replace('_DUP','')}</span>
            <CopyButton text={c.case_number.replace('_DUP','')} />
          </div>
          <button onClick={onClose} className="p-1 rounded" style={{ color:'var(--text-muted)' }}><X size={16}/></button>
        </div>
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Date</div>
              <div className="text-sm font-mono" style={{ color:'var(--text-primary)' }}>{c.date}</div>
            </div>
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Duration</div>
              <div className="text-sm font-mono" style={{ color:'var(--text-primary)' }}>{Math.round(c.duration_sec/60*10)/10}m</div>
            </div>
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Type</div>
              <div className="text-sm font-bold" style={{ color: isEsc?'#e05555':'#34c98a' }}>{c.case_type}</div>
            </div>
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Avoidable</div>
              <div className="text-sm" style={{ color:'var(--text-primary)' }}>{c.avoidable || '—'}</div>
            </div>
          </div>
          {c.avoidable_category && (
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Category</div>
              <div className="text-sm" style={{ color:'var(--text-primary)' }}>{c.avoidable_category}</div>
            </div>
          )}
          {c.comments && (
            <div className="rounded-lg p-3" style={{ background:'var(--bg-secondary)' }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color:'var(--text-muted)' }}>Comments</div>
              <div className="text-sm" style={{ color:'var(--text-primary)' }}>{c.comments}</div>
            </div>
          )}
        </div>
      </div>
...
export default function AgentTracker({ accessConfig }) {
  ...
  function handleCancel() { cancelCase(); setStep('idle'); setError(''); }
  ...
  function handleStartCase() {
    ...
    setIsDuplicate(false); setDuplicateTimes(1); setComments(''); setError('');
  }
  ...
  async function handleCloseCase() {
    ...
    const ok = await closeCase({ escalation, avoidable, avoidableCategory, isDuplicate, duplicateTimes, comments });
  ...
  async function handleSearch(e) {
    ...
    await searchCase(searchQuery.trim());
  }

  async function handleDateChange(date) {
    setSelectedDate(date);
    if (date === today) { setShowSearch(false); }
    else await loadCasesForDate(date);
  }

  const casesToShow = (showSearch || selectedDate !== today) ? displayCases : todayCases;
  const sectionLabel = showSearch && viewDate==='search'
    ? `Search results for "${searchQuery}"`
    : selectedDate !== today ? `Cases for ${selectedDate}` : `Today's Cases — ${totalCases} total`;
  ...
  return (
    <div className="min-h-screen" style={{ background:'var(--bg-primary)' }}>
      ...
      {/* Header */}
      <header className="sticky top-0 z-40" style={{ background:'var(--bg-primary)', borderBottom:'1px solid var(--border)' }}>
        <div className="px-6 py-3 flex items-center gap-4">
          ...
          {activity && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold cursor-pointer"
              style={{ background: 'rgba(55,154,171,.15)', color: accent }}
              onClick={endActivity}>
              <span>{ACTIVITY_TYPES.find(a=>a.key===activity.type)?.icon}</span>
              {fmtTime(actElapsed)} · tap to end
            </div>
          )}
      ...
      <div className="p-6">
        <div className="max-w-5xl mx-auto space-y-4">
            ...
            <div className="rounded-2xl border-2 overflow-hidden" style={{ borderColor: accent, background:'var(--bg-card)' }}>
              ...
              <div className="p-6 grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  {/* Comments */}
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color:'var(--text-muted)' }}>Comments (optional)</div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-4">
                <button onClick={handleNewCase}
                  className="py-6 rounded-2xl text-sm font-bold flex flex-col items-center justify-center gap-2 transition-all hover:opacity-90"
                  style={{ background: accent, color:'#fff' }}>
                  <Plus size={28}/> New Case
                </button>
                {ACTIVITY_TYPES.map(act => (
                  <button key={act.key}
                    onClick={() => activity?.type===act.key ? endActivity() : startActivity(act.key)}
                    disabled={!!activity && activity.type!==act.key}
                    className="py-4 rounded-2xl text-sm font-bold flex flex-col items-center justify-center gap-1.5 transition-all"
                    style={{
                      background: activity?.type===act.key ? accent : 'var(--bg-card)',
                      color: activity?.type===act.key ? '#fff' : accent,
                      border: `2px solid ${accent}`,
                      opacity: (activity && activity.type!==act.key) ? 0.3 : 1
                    }}>
                    <span style={{ fontSize:20 }}>{act.icon}</span>
                    <span>{activity?.type===act.key ? `${fmtTime(actElapsed)}` : act.label}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ background:'var(--bg-card)', borderColor:'var(--border)' }}>
            <div className="px-5 py-3 border-b flex items-center gap-3" style={{ borderColor:'var(--border)', background:'var(--bg-secondary)' }}>
              <span className="text-xs font-semibold uppercase tracking-widest flex-1" style={{ color:'var(--text-muted)' }}>
                {sectionLabel}
              </span>
              {/* Date picker */}
              <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowSearch(!showSearch)}
                ...
                style={{ background: showSearch ? accent : 'transparent', color: showSearch ? '#fff' : 'var(--text-muted)' }}>
                <Search size={14}/>
              </button>
            </div>

            {showSearch && (
              <form onSubmit={handleSearch} className="px-4 py-3 border-b flex gap-2" style={{ borderColor:'var(--border)' }}>
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  placeholder="Search by case number..."
                  className="flex-1 rounded-lg px-3 py-2 text-sm border outline-none font-mono"
                  style={{ background:'var(--bg-secondary)', borderColor:'var(--border)', color:'var(--text-primary)' }}
                />
                <button type="submit" className="px-4 py-2 rounded-lg text-sm font-semibold"
                  style={{ background: accent, color:'#fff' }}>Search</button>
                <button type="button" onClick={() => { setShowSearch(false); setSearchQuery(''); handleDateChange(today); }}
            ...
            {!loading && casesToShow.length === 0 && (
              <div className="text-center py-8 text-sm" style={{ color:'var(--text-muted)' }}>No cases found</div>
            )}

            {!loading && casesToShow.length > 0 && (
              <div className="p-4 grid grid-cols-3 gap-3">
                {[...casesToShow].reverse().map(c => {
                  const isDup = c.case_number.endsWith('_DUP');
                  const isEsc = c.case_type==='Escalation';
                  return (
                    <div key={c.id} className="rounded-xl border px-4 py-3 flex items-center gap-3 cursor-pointer transition-all hover:border-current"
